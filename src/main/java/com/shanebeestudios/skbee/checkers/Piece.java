...
public class Piece {
    ...
    public ArrayList<int[]> getMoves(Checkers checkers) {
        ...
        if ((direction == -1 && y > 0) || (direction == 1 && y < 7)) {
            if (x > 0) piece = checkers.getPiece(x - 1, y + direction);
            if (x > 0 && piece.isNull())
                moves.add(new int[] {x - 1, y + direction});
            else if (x > 1 && ((direction == -1 && y > 1) || (direction == 1 && y < 6)) && checkers.getPiece(x - 2, y + direction * 2).isNull() && piece.color != color)
                moves.add(new int[] {x - 2, y + direction * 2});

            if (x < 7) piece = checkers.getPiece(x + 1, y + direction);
            if (x < 7 && piece.isNull())
                moves.add(new int[]{x + 1, y + direction});
            else if (x < 6 && ((direction == -1 && y > 1) || (direction == 1 && y < 6)) && checkers.getPiece(x + 2, y + direction * 2).isNull() && piece.color != color)
                moves.add(new int[]{x + 2, y + direction * 2});
        }
